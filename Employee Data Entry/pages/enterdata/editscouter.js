...
export const getServerSideProps = async (context) => {
  const { data: users } = await axios.get(`${process.env.URL_PAGE}/api/scouter/scouter`)
...
export default function EditScouter({totalUsernamesArray}) {
  ...
  React.useEffect( async () => {    
    ...
    axios.get(`/api/scouter/user?username=` + currentusername).then(res => {
      const { data: u } = res;
      if(u.length > 0) {
      ...
      <section className={styles.layout}>
        ...
        <div className={styles.main}>
            <div className={styles.centerChildren}>
                <h1 style={textStyle}>EDITAR UN SCOUTER</h1>
